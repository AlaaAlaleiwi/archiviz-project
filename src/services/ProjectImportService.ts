import type { Edge, Graph, Language, NodeData, NodeType } from "../types";
...
function shouldReadFile(path: string) {
  const parts = splitPath(path).map((part) => part.toLowerCase());
...
function detectLanguage(files: ProjectFile[]): Language {
  if (files.some((file) => file.relativePath.endsWith(".ts") || file.relativePath.endsWith(".tsx") || file.relativePath === "tsconfig.json")) {
    return "typescript";
  }

  if (files.some((file) => file.relativePath.endsWith(".js") || file.relativePath.endsWith(".jsx") || file.relativePath === "package.json")) {
    return "javascript";
  }

  if (files.some((file) => file.relativePath.endsWith(".py") || file.relativePath === "requirements.txt" || file.relativePath === "pyproject.toml")) {
    return "python";
  }

  if (files.some((file) => file.relativePath.endsWith(".java") || file.relativePath === "pom.xml" || file.relativePath.startsWith("build.gradle"))) {
    return "java";
  }
...
function layoutNodes(buckets: Bucket[]): NodeData[] {
...
export class ProjectImportService {
  async importFromFiles(fileList: FileList | File[]) {
    ...
    for (const file of files) {
      ...
      if (!relativePath || !shouldReadFile(fullPath)) continue;
    ...
    const nodes = layoutNodes(buckets);
